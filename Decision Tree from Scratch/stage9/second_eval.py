...
class DecisionTree:
    ...
    def _split_with_num(self, node, data, res):
        ...
        if len(data) <= self.min_el or gini == 0.0 or same:
            ...
            w_gini, feature, val, left, right = self._split_fun_with_num(data, res)
...
features_train_stage9 = train_data[["Pclass", "Sex", "SibSp", "Parch", "Age", "Fare"]]
target_train_stage9 = train_data["Survived"].values
features_test_stage9 = test_data[["Pclass", "Sex", "SibSp", "Parch", "Age", "Fare"]]
target_test_stage9 = test_data["Survived"].values
tree = DecisionTree(min_el=74, numeric_features=["Age", "Fare"])
tree.fit(features_train_stage9, target_train_stage9)
y_pred = tree.predict(features_test_stage9)
tn, fp, fn, tp = confusion_matrix(target_test_stage9, y_pred, normalize='true').ravel()
